...
Add an optional `id` field to relationship objects (`RelationshipBase`, surfaced as `RelationshipSchemaLevel` and `RelationshipPropertyLevel`) in ODCS, completing the work started in [RFC-0026a](../odcs-v3.1.0/0026a-reference-id.md). This is a narrow, additive change that brings relationship objects in line with every other referenceable array-item object in the standard.

## Motivation

[RFC-0026a](../odcs-v3.1.0/0026a-reference-id.md) introduced optional, stable `id` fields across referenceable array-item objects in ODCS — servers, schema objects, properties, support channels, roles, SLA properties, custom properties, and quality rules. The goal was to enable robust, position-independent references that survive name changes and array reordering.
...
* [RFC-0026a: Adding ID Field for Stable References](../odcs-v3.1.0/0026a-reference-id.md)
