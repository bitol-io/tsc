# Vendor Attribution for Custom Properties
...
This RFC adds an optional `vendor` field to the shared `customProperties` structure used across the Bitol standards (ODCS, ODPS, and OORS).
...
The Bitol standards already support `customProperties` and allow `value` to contain structured data.
...
Add an optional `vendor` field to the shared `customProperties` structure.
...
**Approved by the TSC on 2026-06-30** for inclusion in ODCS v3.2.0, ODPS v1.1.0, and OORS v1.0.0.
