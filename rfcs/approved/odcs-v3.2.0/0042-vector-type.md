...
| ----------------------------------------- | -------- | ------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------ |
| `dimensions`                              | Yes      | integer | The fixed length of the vector. Positive integer. Examples: `384`, `768`, `1024`, `1536`, `3072`.                                                                                                                                                                                          |
...
| `elementType`                             | No       | string  | Numeric type of each element. One of `bfloat16`, `binary`, `float16`, `float32` (default), `float64`, `int8`, `uint8`. `binary` means each element is one bit (used by binary-quantized vectors).                                                                                           |
...
| HCL Informix               | `lvector_embedding (1536)` (Vector Blade; dims 2–1536 at first release)                                                                                                    |
...
**Resolved:** the TSC adopted **`elementType`** on 2026-06-30, for clarity and to keep `logicalTypeOptions` self-contained (avoiding a second meaning for `physicalType`).
...
**Resolved:** the TSC adopted **`dimensions`** on 2026-06-30 — the clearest term for the N-dimensional-space mental model standard in ML/retrieval.
...
**Approved by the TSC on 2026-06-30** for inclusion in ODCS v3.2.0. Field names ratified at the same vote: `dimensions` (over `size`/`numDimensions`/`dim`) and `elementType` (over `physicalType`).
...
| **HCL**         | [HCL Informix Vector Blade](https://www.hcl-software.com/informix)                                                                                                                      | `lvector_embedding (N)`                                         | 2–1536 ¹   | `float` (float64)                                            | cosine, dot product, L2                       |
...
¹ HCL Informix Vector Blade supports 2–1536 dimensions at its first release (June 2026); the maximum is expected to be raised in a later release based on feedback.
...
| Date | Author | Change |
| ---- | ------ | ------ |
| 2026-05-28 | Jean-Georges Perrin | Added HCL Informix Vector Blade to the physical type mapping and vendor landscape (`lvector_embedding (1536)`, 2–1536 dims at first release, float64, cosine/dot product/L2). |
